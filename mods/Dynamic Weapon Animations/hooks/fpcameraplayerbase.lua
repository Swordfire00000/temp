...
Hooks:PostHook(FPCameraPlayerBase, "update", "immersive_fpcamera", function(self, unit, t, dt)
	...
	local insight_mul = curr_state:in_steelsight() and .10 or 1.37
	...
	local lp_st_spd = deltaTime * (stance_name == "steelsight" and 11 or 5.5)

	stance_pos = stance_pos or Vector3()
	if curr_state:_is_meleeing() or curr_state:_is_throwing_projectile() then
		mvector3.lerp(stance_pos, stance_pos, Vector3(wep_stance.translation.x, 10, wep_stance.translation.z), lp_st_spd)
	else
	...
	local lp_sway_spd = deltaTime * 16
	...
	local ply_max_spd = 0
	if not mvmnt:in_air() and (mvmnt:current_state_name() ~= "jerry1" and mvmnt:current_state_name() ~= "jerry2") and (stance_name == "standard" or stance_name == "steelsight") then
	...
	local lp_tilt_spd = deltaTime * 10
	...
	mvector3.lerp(tilt_pos, tilt_pos, Vector3(stance_name == "steelsight" and (tilt_wobble / 9) or tilt_wobble, 0, tilt_wobble / 2.5), lp_tilt_spd)
	...
	local lp_offset_spd = deltaTime * 9
	...
	if not wep.akimbo then
		...
		local offset_pos = Vector3(-8.5 * stance_offset, -5 * stance_offset, -16.5 * stance_offset)
		local offset_ang = Rotation(-2.5 * stance_offset, 5.5 * stance_offset, -35 * stance_offset)
